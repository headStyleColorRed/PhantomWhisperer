...
fn parse_aprs_packet(bytes: &[u8]) -> Result<AprsPacket, String> {
    println!("Starting parse_aprs_packet");
    if bytes.len() < 16 {
    ...
    println!("Decoded addresses: Destination: {}, Source: {}", destination, source);
    ...
    println!("Found info_start at index: {}", info_start);

    if info_start + 2 >= bytes.len() {
        println!("No information field");
    ...
    println!("Extracted information field: {}", information);
    ...
    let calculated_crc = crc.checksum(&bytes[..info_end]);
    let packet_crc = u16::from_be_bytes([bytes[info_end], bytes[info_end + 1]]);
    println!("CRC check: calculated {:04X}, found {:04X}", calculated_crc, packet_crc);
