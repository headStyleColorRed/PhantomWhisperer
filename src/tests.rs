...
// The encode_modulate_decode function takes a string input, encodes it, modulates it, and then decodes it.
fn encode_modulate_decode(input: &str) -> Result<String, Box<dyn std::error::Error>> {
    let encoded_file = NamedTempFile::new()?;
    let modulated_file = NamedTempFile::new()?;

    encoder::encode_message(input, encoded_file.path().to_str().unwrap())?;
    modulator::modulate_file(encoded_file.path().to_str().unwrap(), modulated_file.path().to_str().unwrap())?;

    decoder::decode_file(modulated_file.path().to_str().unwrap())
}
...
fn test_size_encoding() {
    ...
    let result = encode_modulate_decode(input).unwrap();
...
fn test_noise_resistance() {
    ...
    let mut noisy_samples: Vec<i16> = samples.iter().map(|&s| {
...
fn test_performance() {
    ...
    let _ = encode_modulate_decode(&input).unwrap();
    let duration = start.elapsed();

    println!("Time taken to encode, modulate, and decode 100,000 characters: {:?}", duration);
